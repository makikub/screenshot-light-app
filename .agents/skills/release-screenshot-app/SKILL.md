...
`scripts/release.sh` の仕組みは `AGENTS.md` の **Release Workflow** セクション参照。
...
**完了判定**:
- `Done! DMG: ...` が出れば成功
- `did not succeed` / `status: Invalid` / `ERROR:` が出たら失敗 → AGENTS.md の
...
コミットメッセージは `release: v<short>` で固定（AGENTS.md の commit 規約に従う）。
